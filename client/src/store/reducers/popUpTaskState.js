...
export const popUpTaskState = createSlice({
  ...
  reducers: {
    changePopUpTaskState: (state, action) => {
      ...
      state.activeTask = action.payload.popUpTaskActive ? action.payload.activeTask : {}
    },
    confirmStageTask: (state, action) => {
      state.activeTask.stages.forEach(stage => {
        if(stage.timeStart === action.payload.stage.timeStart && stage.description === action.payload.stage.description) {
